...
- Sequence length=64
- Fully patched compute_metrics (float→int fix + flattening)
"""
...
# -------------------------
# FINAL PATCHED compute_metrics (float→int + flatten fix)
# -------------------------
def compute_metrics(eval_pred):
    ...
    for p in preds_ids:
        ...
        # flatten nested lists like [[[…]]]
        while isinstance(p, list) and len(p) > 0 and isinstance(p[0], list):
            p = p[0]

        # 🔥 FIX #1 — convert floats → ints
        p = [int(x) if not isinstance(x, bool) else 0 for x in p]

        # 🔥 FIX #2 — replace negative or invalid token IDs
        pad_id = tokenizer.pad_token_id
        p = [x if x >= 0 and x < tokenizer.vocab_size else pad_id for x in p]
    ...
    sari_value = corpus_sari(sources, preds, refs)
    fkgl_value = float(np.mean([
        textstat.flesch_kincaid_grade(p) if p.strip() else 0 for p in preds
    ]))

    return {"sari": sari_value, "fkgl": fkgl_value}
